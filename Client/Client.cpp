...
void Client::sendMessage()
{
    ...
    wsSocket->write(QString("%2").arg(message));
...
{
    if(message.compare("CONNECTED")==0 || message.compare("DISCONNECTED")==0)
    {
...
void Client::on_pbAdd_clicked()
{
    //per il primo elemento
    if(first)
    {
    userdata+=ui->inx->text()+","+ui->iny->text();
    ui->tePoints->setText(userdata);
    first=false;
    }
    else
    {
        userdata+="_"+ui->inx->text()+","+ui->iny->text();
...
void Client::on_pbClear_clicked()
{
    userdata.clear();
    ui->tePoints->setText("");
}
