...
void Server::processMessage(QString frame)
{
    ...
    QString data = toReadableAscii(frame);


    bool ok;
    QVector<QPointF> points;
    for(QString token:data.split("_"))
    {
        QPointF pnt;
        QList<QString> coords= token.split(",");

        pnt.setX(coords.at(0).toInt(&ok,10));
        pnt.setY(coords.at(1).toInt(&ok,10));
        points.append(pnt);


        }

    LSR LeastSquareRegression;
    vector<QPointF> regression=LeastSquareRegression.regression(points,6);
    ...
    {
        QPointF el = regression.at(i);
        elements+=",";
        elements+=QString::number(el.x());
        elements+=",";
        elements+=QString::number(el.y());
